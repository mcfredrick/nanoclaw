...
| `src/channels/signal.ts` | Signal connection, auth, send/receive |
...
1. Create `src/channels/{name}.ts` implementing the `Channel` interface from `src/types.ts` (see `src/channels/signal.ts` for reference)
...
2. Ask: "Should Telegram messages create separate conversation contexts, or share with Signal groups?"
3. Create `src/channels/telegram.ts` implementing the `Channel` interface (see `src/channels/signal.ts`)
